...
MODEL_CARD_REGISTRY = Registry("model cards")
...
def discover_model_cards(search_roots, *, model_card_registry=None, preset_registry=None):
    ...
    model_card_registry, preset_registry = _registries(model_card_registry, preset_registry)
...
def _registries(model_card_registry, preset_registry):
    if model_card_registry is None:
        model_card_registry = MODEL_CARD_REGISTRY
    if preset_registry is None:
        from .presets import PRESETS_REGISTRY

        preset_registry = PRESETS_REGISTRY
    return model_card_registry, preset_registry
