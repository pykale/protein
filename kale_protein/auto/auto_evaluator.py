...
class MultiMetricEvaluator:
    def __init__(self, config):
        ...
        self.metrics = [
            (name, EVALUATOR_REGISTRY.get((task, name))()) for name in metric_names
        ]
...
def _import_task_module(task, module):
    ...
    try:
        importlib.import_module(f"kale_protein.tasks.{task}.{module}")
    except ModuleNotFoundError as error:
        if error.name == f"kale_protein.tasks.{task}":
            return
        raise
